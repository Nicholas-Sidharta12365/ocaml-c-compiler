...
module Ast = struct
  type t = 
    ...
    | Infix of {left: t; operator: Token.t; right: t}
    | Token of Token.t
    | Declaration of {type_def: t; name: t; value: t option}
    | ExpressionStatement of t
    | Return of t
    | Program of {statements: t list}
  [@@deriving sexp_of]
end
...
let rec eval_expression env = function
  | Ast.Integer n -> Int n
  | Ast.Identifier id -> Env.find_exn env id.identifier
  | Ast.Infix {left; operator; right} ->
      let lval = eval_expression env left in
      let rval = eval_expression env right in
      eval_infix_op operator lval rval
  | _ -> failwith "Not implemented"

and eval_infix_op op lval rval =
  match (op, lval, rval) with
  | (Token.Plus, Int l, Int r) -> Int (l + r)
  | _ -> failwith "Invalid operation or operand types"

let rec eval_statement env = function
  | Ast.Declaration {type_def; name; value} ->
      let value = Option.value_map value ~default:(Int 0) ~f:(eval_expression env) in
      (Env.set env ~key:name.identifier ~data:value, None)
  | Ast.ExpressionStatement expr ->
      (env, Some (eval_expression env expr))
  | Ast.Return expr ->
      (env, Some (eval_expression env expr))
  | _ -> failwith "Not implemented"

let eval_program (Ast.Program {statements}) =
  let rec loop env = function
    | [] -> None
    | stmt::stmts ->
        let (env', result) = eval_statement env stmt in
        match result with
        | Some _ as r -> r
        | None -> loop env' stmts
  in
  loop empty_env statements *)

open Base

let monkey_true = evalObject.monkey_true
let monkey_false = evalObject.monkey_false
...
let obj_is_truthy (obj : evalObject.t) =
  match obj with
  | evalObject.Boolean res -> res
  | Null -> false
  | _ -> true
;;

let obj_equal a b =
  (* let open evalObject in *)
  (* match a, b with *)
...
let builtins =
  ...
  @@ builtin_fn (function
       ...
       | hd :: _ -> Fmt.error "push: bad head value: %a" evalObject.pp hd
       | _ -> Error "push: must be an array");
  Environment.set env "puts"
  @@ builtin_fn (fun args ->
       List.iter args ~f:(fun arg -> Fmt.pr "%a@." evalObject.pp arg);
       Ok Null);
  env
;;
...
and eval_expr expr env =
  ...
  | Ast.FunctionLiteral { parameters; body } ->
    Ok (evalObject.Function { parameters; body; env = Environment.enclosed env })
  | Ast.Call { fn; args } ->
  ...
  | Ast.Hash hash ->
    let* elements =
      List.fold_until
        ...
        ~finish:(fun elts -> Ok (List.rev elts))
    in
    Ok (evalObject.Hash elements)
  | Ast.Index { left; index } ->
    let* left = eval_expr left env in
    let* right = eval_expr index env in
    begin
      match left, right with
      | evalObject.Array lst, evalObject.Integer int ->
...
and apply_function fn args =
  ...
  | obj -> Fmt.error "bad function: %a" evalObject.pp obj
...
and eval_infix operator left right =
  ...
  | _, left, right ->
    Fmt.error
      ...
      (evalObject.show left)
      (Token.show operator)
      (evalObject.show right)

and eval_str_infix operator left right =
  match operator with
  | Token.Plus -> Ok (evalObject.String (left ^ right))
  | _ -> Error "unexpected str infix"

and eval_integer_infix operator left right =
  let make_int op a b = evalObject.Integer (op a b) in
  let make_bool op a b = evalObject.Boolean (op a b) in
  let* maker =
    match operator with
...
module Test = struct
  ...
  let expect_bool input =
    ...
    | Ok expr -> Fmt.pr "WRONG TYPES: %s@." (evalObject.show expr)
    | Error msg -> Fmt.failwith "%s" msg
  ;;
  ...
  let expect_err input =
    match eval_input input with
    | Ok expr -> Fmt.failwith "should have failed: %s@." (evalObject.show expr)
